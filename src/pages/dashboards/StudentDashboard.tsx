...
export const StudentDashboard = () => {
  ...
  const handleSubmitProject = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const nextProject: Project = {
      ...
      pdfUrl: formState.pdfFile ? URL.createObjectURL(formState.pdfFile) : undefined,
    };
    ...
    if (created) {
      ...
      setFormState({ title: '', abstract: '', dept: 'CICS', program: 'BSIT', year: '2026', keywords: '', tech_stack: '', lessons_learned: '', githubLink: '', pdfFile: null });
        ...
        return (
          <SectionCard title="Project Submission Form" subtitle="Submit a project draft for review and approval." count={0}>
            <form onSubmit={handleSubmitProject} className="space-y-8 rounded-[32px] bg-slate-950/40 border border-white/5 p-10">
              ...
              {/* NEW: PDF Upload and GitHub Link */}
              <div className="grid gap-6 lg:grid-cols-2">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-[0.35em] text-slate-500 mb-2">GitHub Link (optional)</label>
                  <input
                    type="url"
                    value={formState.githubLink}
                    onChange={(e) => setFormState((current) => ({ ...current, githubLink: e.target.value }))}
                    placeholder="https://github.com/yourusername/project"
                    className="w-full rounded-[24px] border border-white/10 bg-slate-950/90 px-5 py-4 text-sm text-white outline-none"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-[0.35em] text-slate-500 mb-2">Upload PDF (optional)</label>
                  <input
                    type="file"
                    accept="application/pdf"
                    onChange={(e) => {
                      const file = e.target.files?.[0] || null;
                      setFormState((current) => ({ ...current, pdfFile: file }));
                    }}
                    className="w-full rounded-[24px] border border-white/10 bg-slate-950/90 px-5 py-4 text-sm text-white file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-orange-500 file:text-slate-950 file:font-black file:uppercase file:tracking-[0.2em] file:text-xs outline-none"
                  />
                  {formState.pdfFile && (
                    <p className="mt-2 text-xs text-emerald-400">✓ {formState.pdfFile.name} selected</p>
                  )}
                </div>
              </div>
...
// ... (rest of the file - SectionCard, InputField, TextAreaField, StatCard, EmptyState, ProjectCard, NavItem - same as before)
...
const InputField = ({ 
  ...
  <label className="block text-sm text-slate-200">
    <span className="mb-3 block text-[10px] font-black uppercase tracking-[0.35em] text-slate-500">
...
const TextAreaField = ({ 
  ...
  <label className="block text-sm text-slate-200">
    ...
    <textarea 
      ...
      onChange={(e) => onChange(e.target.value)} 
      required={required}
      className="w-full min-h-[160px] rounded-[24px] border border-white/10 bg-slate-950/90 px-5 py-4 text-sm text-white outline-none resize-none" 
    />
  </label>
);
  ...
  <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-[32px] border border-white/10 bg-slate-950/40 p-8 shadow-xl">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
      ...
      <div className="flex flex-col gap-3 min-w-[220px]">
        ...
        {project.githubLink && (
          <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="rounded-3xl bg-white/5 px-5 py-4 text-left text-sm uppercase tracking-[0.35em] text-slate-200 hover:bg-white/10 flex items-center gap-2">
            View on GitHub ↗
          </a>
        )}
        {project.pdfUrl && (
          <a href={project.pdfUrl} download={`${project.title}.pdf`} className="rounded-3xl bg-emerald-500/10 px-5 py-4 text-left text-sm uppercase tracking-[0.35em] text-emerald-300 hover:bg-emerald-500/20 flex items-center gap-2">
            Download PDF ⬇
          </a>
        )}
      </div>

...
const PreviewPane = ({ project }: { project: Project }) => (
  <div className="rounded-[32px] border border-white/10 bg-slate-950/40 p-10 shadow-xl">
    ...
    <div className="space-y-6">
      ...
      {/* NEW: Big GitHub & PDF Buttons in Preview */}
      {(project.githubLink || project.pdfUrl) && (
        <div className="pt-4 border-t border-white/10">
          <h4 className="text-sm uppercase tracking-[0.35em] text-slate-500 mb-4">Project Files</h4>
          <div className="flex flex-wrap gap-4">
            {project.githubLink && (
              <a 
                ...
                className="flex-1 min-w-[200px] rounded-3xl bg-white/5 hover:bg-white/10 border border-white/10 px-8 py-5 text-center transition-all group"
              >
                <div className="text-orange-400 group-hover:scale-110 transition-transform inline-block">↗</div>
                <div className="mt-1 text-lg font-black uppercase tracking-[0.2em]">View on GitHub</div>
                <div className="text-[10px] text-slate-500 mt-1">Open repository</div>
              </a>
            )}
            {project.pdfUrl && (
              <a 
                ...
                className="flex-1 min-w-[200px] rounded-3xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 px-8 py-5 text-center transition-all group"
              >
                <div className="text-emerald-400 group-hover:scale-110 transition-transform inline-block">⬇</div>
                <div className="mt-1 text-lg font-black uppercase tracking-[0.2em]">Download PDF</div>
                <div className="text-[10px] text-emerald-600 mt-1">Full project document</div>
              </a>
            )}
          </div>
        </div>
...
const UserRow = ({ user, onChangeRole }: { user: AppUser; onChangeRole: (role: AppUser['role']) => void }) => (
  <div className="flex flex-col gap-3 rounded-[32px] border border-white/10 bg-slate-950/40 p-6 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="flex flex-wrap gap-3 items-center">
      ...
      <select 
        value={user.role} 
        onChange={(event) => onChangeRole(event.target.value as AppUser['role'])} 
        className="rounded-3xl bg-slate-900/90 border border-white/10 px-4 py-3 text-sm text-white outline-none"
      >
